# Guga Chapichadze

**Operations operator building AI-native automation for scale-ups.** Based in Barcelona.

9 years running operations at architecture and tech firms — built the systems (project profitability calculators, 9-criterion KPI matrices, onboarding manuals, full project lifecycles in Monday.com/ClickUp) that turn 25–30 person teams from chaotic to predictable. Now applying the same discipline to AI-native tooling: automation engines, vertical SaaS, fractional consulting.

---

## Currently building

- **AOS — Architecture Operating System.** Vertical SaaS for architecture studios. FastAPI + async SQLAlchemy + Postgres. DMARK Architecture as committed design partner.
- **[guga-consulting.com](https://guga-consulting.com).** Fractional COO practice. Free Ops Health Check (25 questions → radar chart) and OpsOS wizard live.
- **This portfolio.** Public artifacts of the systems above — pinned repos ship over the next two weeks.

## Stack

Python · FastAPI · Claude API · Postgres · SQLite · Alembic · uv · Docker · n8n · Netlify · Monday.com · ClickUp · Obsidian

## Languages

Georgian (native) · English (C2) · Russian (C1) · Spanish (B1, developing)

## Find me

- [guga-consulting.com](https://guga-consulting.com)
- LinkedIn: www.linkedin.com/in/gugachapichadze
- gugachapichadze@gmail.com
- **[job-hunt-intelligence-engine](https://github.com/gugachapichadze-collab/job-hunt-intelligence-engine).** Public Python engine — scores ~100 LinkedIn jobs/day, surfaces ~3 APPLY matches/week at €0.001/job. v3.1.15 in production since April 2026.
